...
5. [Ouvrir et fermer un document](#ouvrir)
6. [Navigation dans le document](#navigation)
7. [Zoom et affichage](#zoom)
8. [Modifier le texte existant](#modifier-texte)
9. [Insérer du texte](#inserer-texte)
10. [Annotations](#annotations)
11. [Insérer une image](#inserer-image)
12. [Formulaires PDF](#formulaires)
13. [Reconnaissance de caractères (OCR)](#ocr)
14. [Gestion des pages — Réorganiser / Fusionner / Fractionner](#pages)
15. [En-têtes et pieds de page](#entetes)
16. [Filigrane](#filigrane)
17. [Tampon texte](#tampon-texte)
18. [Tampon image — logo et signature](#tampon-image)
19. [Intégration Windows — clic droit](#windows)
20. [Métadonnées du document](#metadata)
21. [Compression PDF](#compression)
22. [Protection par mot de passe](#protection)
23. [Signature numérique](#signature)
24. [Recherche de texte](#recherche)
25. [Extraction de contenu](#extraction)
26. [Enregistrer le document](#enregistrer)
27. [Annuler / Rétablir](#annuler)
28. [Thèmes et langue](#langue)
29. [Raccourcis clavier](#raccourcis)

---

> **Nouveautés v1.5.8** : menu **Outils** entièrement organisé en sous-menus (*Insérer / Organiser / Extraire / Sécuriser / OCR*) ; mode **défilement continu** (`Ctrl+Maj+C`) ; **barre de recherche inline** ; menu **Préférences** centralisé (`Édition > Préférences`) ; couche OCR en **Unicode complet** (cyrillique, CJK, arabe…) ; **éditeur OCR inline** (double-clic sur une ligne OCR pour la corriger sans popup).
>
> **v1.5.0** : dialogue **Préférences**, **Apparence**, **Plein écran**, duplication de page, mode de visualisation continu, recherche inline.
>
> **v1.4.1** : menu contextuel **Combiner dans PDF Editor** (sélection multiple de fichiers → dialogue de réorganisation pré-chargé) · dialogue *Intégration Windows* revu avec deux sections activables séparément.
>
> **v1.4.0** : navigation page suivante/précédente via l'ascenseur et la molette · extraction de texte avec choix de l'intervalle de pages · popup de résumé après extraction · panneau *Outils* aligné sur le menu Outils · *À propos* enrichi.
>
...
![Main window](../website/screenshots/fr/main_window.png)


```
┌─────────────────────────────────────────────────────────────────┐
│  Menu  (Fichier · Édition · Affichage · Outils · Aide)          │
├─────────────────────────────────────────────────────────────────┤
...
- **Panneau latéral gauche** : deux onglets — *Pages* et *Outils*. Masquable avec `F4`.
- **Visionneuse** : page unique par défaut, ou mode *défilement continu* (`Affichage → Défilement continu`, `Ctrl+Maj+C`).
- **Barre de recherche** : apparaît en haut de la zone de lecture (`Ctrl+F`) et se referme avec `Échap`.
...
| **Outils** | Section *Outils* (même ordre que le menu) · section *Annotations* · section *Raccourcis* |
...
| **Édition** | Annuler, Rétablir, Rechercher, **Préférences** |
| **Affichage** | Zoom, Panneau, Défilement continu, Plein écran, Thème, Apparence |
| **Outils** | Actions regroupées en sous-menus : *Insérer*, *Organiser*, *Extraire*, *Sécuriser*, *OCR* |
| **Aide** | Manuel, Raccourcis, Signaler un bug, Vérifier les mises à jour, À propos |

> La langue, la licence, l'intégration Windows et l'apparence sont désormais centralisées dans **Édition > Préférences**.
...
Tous les réglages d'application sont regroupés dans un seul dialogue accessible par **Édition → Préférences** (`Ctrl+,`) :

| Onglet | Contenu |
|--------|---------|
| **Langue** | Choix de la langue de l'interface (redémarrage proposé) |
| **Aide et raccourcis** | Accès au manuel et récapitulatif des raccourcis |
| **Licence et intégration** | Gestion de la licence, intégration Windows (clic droit) |
| **Apparence** | Personnalisation du thème et des couleurs |

> La langue, l'aide et l'intégration Windows ne sont plus dans des onglets séparés du panneau latéral — elles vivent ici.

---

<a name="ouvrir"></a>
## 5. Ouvrir et fermer un document
...
| **Défilement continu** | `Affichage → Défilement continu` ou `Ctrl+Maj+C` |
| Double-clic en mode continu | Retour à l'affichage page unique à la page cliquée |
...
## 8. Modifier le texte existant

![Text editing](../website/screenshots/fr/text_edit.png)
...
>
> Si la modification ne peut vraiment pas être appliquée (police non modifiable, texte image, bloc vide, échec de l'injection dans le flux), un **message persistant** s'affiche dans la **barre de statut** en bas de la fenêtre (ex. : *"Modification impossible : édition en place échouée…"*).

### Annulation

`Ctrl+Z` pour annuler · `Ctrl+Y` pour rétablir (voir [§27 Annuler / Rétablir](#annuler)).
...
## 11. Insérer une image

**Méthode 1 — Menu**
1. *Outils → Insérer → 🖼 Insérer une image…*
...
![Form filling](../website/screenshots/fr/forms_fill.png)
...
<a name="ocr"></a>
## 13. Reconnaissance de caractères (OCR)

![OCR result](../website/screenshots/fr/ocr_result.png)


**Prérequis** : Tesseract OCR installé (voir [§2](#installation)). Sur la version installée Windows, Tesseract est bundlé.

### Lancer l'OCR

1. *Outils → OCR → 🔤 Reconnaissance de caractères (OCR)…*
...
> La couche texte invisible est encodée en **Unicode complet** (UTF-16) : les caractères cyrilliques, CJK, arabes et tous les autres scripts sont correctement indexés (`Ctrl+F`) et copiables — sans aucune translittération ni perte de caractères.
...
| Option | Description |
|--------|-------------|
| *Outils → OCR → Reconstruire page avec texte natif* | Remplace les éléments texte de la page par du texte PDF natif (meilleure qualité d'édition). |
| *Outils → OCR → Appliquer correction en patch image* | Corrige un texte OCR en modifiant directement l'image de la page (expérimental). |

### Corriger une ligne OCR en un double-clic

Sur un PDF scanné qui contient déjà une couche OCR (par exemple après avoir cliqué sur **Incruster dans le PDF**), vous pouvez corriger une ligne directement depuis la visionneuse :

1. Assurez-vous que l'outil actif est **Modifier texte** (`T`) ou **Sélection** (`S`).
2. **Double-cliquez** sur la ligne de texte scanné à corriger.
3. Un champ d'édition **inline** (sans fenêtre popup) apparaît à la place de la ligne.
4. Corrigez le texte.
5. Appuyez sur **Entrée** pour confirmer, ou sur **Échap** pour annuler.

> La correction est enregistrée comme annotation OCR invisible, indexée par la recherche (`Ctrl+F`) et le copier-coller. Cette opération est **annulable** via `Ctrl+Z`.

> **Astuce** : le double-clic est le moyen le plus rapide de corriger une coquille dans un scan. Si la ligne n'est pas reconnue au clic, lancez d'abord *Outils → OCR → Reconnaissance de caractères (OCR)…* puis cliquez sur **Incruster dans le PDF**.

---

<a name="pages"></a>
## 14. Gestion des pages — Réorganiser / Fusionner / Fractionner

![Merge / organize pages](../website/screenshots/fr/merge_organize.png)


### Réorganiser et fusionner les pages

*Outils → Organiser → ⊕ Réorganiser/Fusionner les pages…* (ou bouton **⊕ Réorganiser/Fusionner** dans la barre)
...
*Outils → Organiser → Dupliquer la page courante* ou `Ctrl+Maj+P`.

> Cette opération est **annulable** via `Ctrl+Z`.

### Supprimer la page courante

*Outils → Organiser → 🗑 Supprimer la page courante* ou `Ctrl+Suppr`.
...
| Action | Méthode |
|--------|---------|
| Tourner +90° | *Outils → Organiser → ↻ Tourner la page (+90°)* ou `R` |
| Tourner -90° | *Outils → Organiser → ↺ Tourner la page (-90°)* ou `Maj+R` |

### Fractionner ce PDF

1. *Outils → Organiser → ✂ Fractionner ce PDF…*
...
## 18. Tampon image — logo et signature

*Outils → Insérer → 🖼 Tampon image…*
...
<a name="windows"></a>
## 19. Intégration Windows — clic droit

![Windows integration settings](../website/screenshots/fr/windows_settings.png)


*Édition → Préférences → Licence et intégration → Intégration Windows (clic droit)…*

PDF Editor propose deux entrées dans le menu contextuel de l'explorateur Windows, toutes deux **activées automatiquement** au premier lancement. Elles sont gérables via *Édition → Préférences → Licence et intégration*.

### Entrées du menu contextuel

| Entrée | Action |
|--------|--------|
| **Ouvrir avec PDF Editor** | Ouvre le fichier sélectionné dans l'application. |
| **Combiner dans PDF Editor** | Sélection multiple de fichiers → ouvre le dialogue de réorganisation pré-chargé avec les PDF choisis prêts à fusionner. |
...
1. *Outils → Sécuriser → 🔒 Protéger par mot de passe…*
...
2. *Outils → Sécuriser → 🔓 Supprimer la protection…*
3. La protection est retirée et enregistrée dans un nouveau fichier.

---

<a name="signature"></a>
## 23. Signature numérique

![Digital signature](../website/screenshots/fr/sign_certificate.png)
...
- Via le menu : *Outils → Sécuriser → ✍ Signer le document…*
- Via le panneau latéral : onglet **Outils** → *✍ Signer le document…*

### Signer

1. *Outils → Sécuriser → ✍ Signer le document…*
2. Renseignez :
...
*Outils → Sécuriser → 🔎 Vérifier les signatures…* (ou bouton *🔎 Vérifier les signatures…* dans le panneau Outils) affiche la liste des signatures et leur statut de validité.
...
<a name="recherche"></a>
## 24. Recherche de texte

1. *Édition → 🔍 Rechercher…* ou `Ctrl+F`.
2. Une **barre de recherche inline** apparaît en haut de la visionneuse.
3. Saisissez le terme à rechercher.
4. Les occurrences sont surlignées ; utilisez **Précédent / Suivant** pour naviguer.
5. Appuyez sur `Échap` ou cliquez sur ✕ pour fermer la barre.
...
*Outils → Extraire → 📄 Extraire le texte…* (ou bouton dans le panneau *Outils*)
...
*Outils → Extraire → 🖼 Extraire les images…* → choisir un dossier de destination.
...
*Affichage → Thème sombre* — active/désactive le thème sombre par un simple interrupteur (sauvegardé dans les préférences).

*Affichage → Apparence…* ouvre le dialogue d'apparence pour des réglages plus fins.

### Langue de l'interface

**Méthode principale** : *Édition → Préférences → Langue*, puis choisissez la langue souhaitée.
...
*Aide* donne un accès rapide à :
- **📖 Manuel utilisateur** (équivalent à `F1`)
- **🐛 Signaler un bug…** — ouvre le formulaire de signalement
- **💡 Suggérer une amélioration…** — ouvre le formulaire de suggestion
- **🔄 Vérifier les mises à jour…**
- La liste des **raccourcis clavier** principaux

L'intégration Windows (clic droit) est désormais configurée depuis *Édition → Préférences → Licence et intégration* (voir [§19](#windows)).

### À propos

*Aide → ℹ À propos* affiche la version, les technologies utilisées et le lien vers le support.
...
| `Ctrl+Z` | Annuler |
| `Ctrl+Y` | Rétablir |
| `Ctrl+F` | Rechercher (barre inline) |
| `Ctrl+,` | Préférences |
...
| `←` | Page précédente |
| `→` | Page suivante |
| `Ctrl+Maj+C` | Activer/désactiver le défilement continu |
...
| `F4` | Afficher/masquer le panneau latéral (Pages/Outils) |
| `F5` | Afficher/masquer la barre d'outils |
| `F11` | Plein écran |
...
| `Ctrl+Maj+P` | Dupliquer la page courante |
| `F1` | Manuel utilisateur |

---

*Manuel mis à jour le 01/07/2026 — PDF Editor v1.5.8*  
